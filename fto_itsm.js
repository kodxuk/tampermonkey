...
(function () {
  ...
  const NET_ACTIVITY_WINDOW_MS = 2000;  // уменьшили окно «свежей сети»
  ...
  const looksAlive=()=>!!document.querySelector('[role="grid"], table, .dataTable, .slickgrid, [id*="menu"], [class*="toolbar"]');
  const hadRecentNetwork=()=>{ try{ const n=performance.now(); return performance.getEntriesByType('resource').some(e => (n - e.responseEnd) <= NET_ACTIVITY_WINDOW_MS); }catch{} return false; };
  const pageLooksBlank=()=>{ const b=document.body; if(!b) return true; const fewNodes = b.children.length<3 && (b.textContent||'').trim().length<40; return fewNodes || !looksAlive(); };
  ...
  addEventListener('storage',(e)=>{ if(e.key===LOCK_KEY && e.newValue){ const safeSup = Math.min(nowMs()+SUPPRESS_MS, nowMs()+2*SUPPRESS_MS); suppressUntil=safeSup; lastSuppressTs=nowMs(); lastBlockAt = lastSuppressTs; blocksCount++; L.lok('storage_event'); passiveRetain('storage'); } });
  ...
  const scheduleWatchdog=()=>{ if(!isWorkingPage()) return; const delay = isEventSearchPage()? (WATCHDOG_MS+AUTOREFRESH_GRACE_MS): WATCHDOG_MS; nextWatchdogAt = nowMs()+delay; L.wd('scheduled',{at:nextWatchdogAt,delay});
    setTimeout(()=>{ 
      const autoRefreshBusy = isEventSearchPage() && hadRecentNetwork();
      if (autoRefreshBusy && looksAlive()) { L.wd('skip_auto_refresh_alive'); return; }
      if (pageLooksBlank()) {
  ...
  const gentleReNavigate = () => {
    ...
    if (isWelcome && rankOk && ageOk) {
      ...
      if (!foreignFreshLock && (recently500 || document.visibilityState==='visible')) {
        L.wel('gentle_nav',{target:last.href,recently500});
